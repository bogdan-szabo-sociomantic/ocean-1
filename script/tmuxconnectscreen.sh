...
#     $screen   = name of screen session to connect to
...
elif [ $location = "asia" ]; then srv_prefix='root@ap-';
else                            echo "location must be one of {eu, us, asia}"; exit 1;
fi
...
do
    if   [ $i -eq 0 ]; then
        ...
        cmd="$cmd tmux new-window 'ssh -t $srv_prefix$server$srv_suffix screen -rx $screen';";
    else
        cmd="$cmd tmux split-window 'ssh -t $srv_prefix$server$srv_suffix screen -rx $screen';";
