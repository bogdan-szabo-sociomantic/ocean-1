...
extern (C) private int accept4(int sockfd, sockaddr* addr, socklen_t* addrlen, int flags = 0);
...
abstract class IIPSocket : InputDevice, IOutputDevice
{
    ...
    public int accept ( ISelectable listening_socket, int flags = 0 )
    {
    ...
    {
        const int[2] flags = [0, SocketFlags.SOCK_NONBLOCK];
...
class IPSocket ( bool IPv6 = false ) : IIPSocket
{
    static if (IPv6)
    {
        alias .sockaddr_in6 InAddr;
    }
    else
    {
        alias .sockaddr_in InAddr;
    }
    ...
    public int accept ( ISelectable listening_socket,
                        ref InAddr remote_address, int flags = 0 )
    {
        socklen_t addrlen;
                        ...
                        ref InAddr remote_address, bool nonblocking )
    {
        ...
        return this.accept(listening_socket,
                           remote_address, flags[nonblocking]);
    }
...
class AddressIPSocket ( bool IPv6 = false ) : IPSocket!(IPv6)
{
    ...
    public override int accept ( ISelectable listening_socket, int flags = 0 )
    {
        scope (exit) this.setAddress();
