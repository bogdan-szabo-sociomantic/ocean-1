...
private
{
    ...
    {
        alias ElementTypeOfArray!(T) ElemTypeOf;
    }
}
...
else
{
    template find_( Elem, Pred = IsEqual!(Elem) )
    {
        ...
        size_t fn( Elem[] buf, Const!(Elem)[] pat, Pred pred = Pred.init )
        {
            if( buf.length == 0 ||
                pat.length == 0 ||
    ...
    size_t find( Element, Pat )( Element[] buf, Pat pat )
    {
        return find_!(Element).fn( buf, pat );
    }

    size_t find( Element, Pat, Pred )( Element[] buf, Pat pat, Pred pred )
    {
        return find_!(Element, Pred).fn( buf, pat, pred );
    }
...
else
{
    ...
    size_t rfind( Element, Pat )( Element[] buf, Pat pat )
    {
        return rfind_!(Element).fn( buf, pat );
    }

    size_t rfind( Element, Pat, Pred )( Element[] buf, Pat pat, Pred pred )
    {
        return rfind_!(Element, Pred).fn( buf, pat, pred );
    }
...
else
{
    template kfind_( Elem, Pred = IsEqual!(Elem) )
    {
        ...
        size_t fn( Const!(Elem)[] buf, Elem pat, Pred pred = Pred.init )
        {
            foreach( size_t pos, Elem cur; buf )
            {
                if( pred( cur, pat ) )
        ...
        size_t fn( Const!(Elem)[] buf, Const!(Elem)[] pat, Pred pred = Pred.init )
        {
            if( buf.length == 0 ||
                pat.length == 0 ||
                buf.length < pat.length )
...
else
{
    ...
    unittest
    {
        void test( char[] buf, cstring pat )
        {
...
version( TangoDoc )
{
     ...
     *
     * Params:
     *  buf  = the array being worked upon.
     ...
     */
    size_t select( Elem[] buf, size_t k, Pred2E pred = Pred2E.init );


    /**
     * The documentation of this method is exactly the same as that of the
     * 'select()' method above, except for the following two points:
     *
     ...
     *            ordering predicate.
     */
    Elem quickselect( Elem[] buf, size_t k, Pred2E pred = Pred2E.init,
                      size_t initial_pivot_index = 0 );
}
else
{
    template quickselect_( Elem, Pred = IsLess!(Elem) )
    {
        static assert( isCallableType!(Pred) );

        Elem fn( Elem[] buf, size_t k, Pred pred = Pred.init,
                 size_t initial_pivot_index = 0 )
        {
            /*
            ...
            size_t partition( Elem[] buf, size_t left, size_t right,
                              size_t pivot_index, Pred pred )
            {
                ...
                auto pivot_value = buf[pivot_index];
                ...
                if( pivot_index != right )
                {
                    ...
                    buf[pivot_index] = buf[right];
                    buf[right] = pivot_value;
                }
                ...
                {
                    if( pred(buf[i], pivot_value) )
                    {
                        if( i != store_index )
                        {
                            typeid(Elem).swap(&buf[i], &buf[store_index]);
                        }
                ...
                {
                    buf[right] = buf[store_index];
                    buf[store_index] = pivot_value;
                }
            ...
            if( initial_pivot_index >= buf.length )
            {
            ...
            while( 1 )
            {
                pivot_index = partition(buf, left, right, pivot_index, pred);
    ...
    template select( Buf, Num )
    {
        size_t select( Buf buf, Num num )
        {
            ...
            quickselect_!(ElemTypeOf!(Buf)).fn( buf, num );

            if( num >= buf.length )
            {
                num = cast(int)(buf.length - 1);
            }
    ...
    template select( Buf, Num, Pred )
    {
        size_t select( Buf buf, Num num, Pred pred )
        {
            ...
            quickselect_!(ElemTypeOf!(Buf), Pred).fn( buf, num, pred );
    ...
    template quickselect( Buf, Num )
    {
        ElemTypeOf!(Buf) quickselect( Buf buf, Num num,
                                      size_t initial_pivot_index = 0 )
        {
            auto pred = IsLess!(ElemTypeOf!(Buf)).init;

            return quickselect_!(ElemTypeOf!(Buf)).fn( buf, num, pred,
                                                       initial_pivot_index );
        }
    }


    template quickselect( Buf, Num, Pred )
    {
        ElemTypeOf!(Buf) quickselect( Buf buf, Num num, Pred pred,
                                      size_t initial_pivot_index = 0 )
        {
            return quickselect_!(ElemTypeOf!(Buf), Pred).fn( buf, num, pred,
                                                          initial_pivot_index );
        }
    }
    ...
    version( UnitTest )
    {
        bool verifySelect( T ) ( T[] buf, size_t k, T expected_value )
        {
            auto kth_element_index = select(buf, k);

            if ( kth_element_index >= buf.length )
            {
                return false;
            }

            if( buf[kth_element_index] != expected_value )
            {
                return false;
            }
    ...
    unittest
    {
         ...
         * know exactly which condition in the 'verifySelect' function was not
         * met. */

        assert(verifySelect("efedcaabca".dup, 5, 'c'));

        assert(verifySelect(['x'], 0, 'x'));
        assert(verifySelect([42], 10, 42));

        assert(verifySelect([7, 3, 4, 1, 9], 0, 1));
        assert(verifySelect([7, 3, 4, 1, 9], 1, 3));
        assert(verifySelect([7, 3, 4, 1, 9], 2, 4));
        assert(verifySelect([7, 3, 4, 1, 9], 3, 7));
        assert(verifySelect([7, 3, 4, 1, 9], 4, 9));
        assert(verifySelect([7, 3, 4, 1, 9], 5, 9));

        assert(verifySelect([7.4, 3.57, 4.2, 1.23, 3.56], 1, 3.56));
