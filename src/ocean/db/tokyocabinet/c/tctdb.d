...
struct TDBIDX                          /* type of structure for a column index */
{
...
struct TCTDB                          /* type of structure for a table database */
{
...
enum TDBFLAGS                                    /* enumeration for additional flags */
{
  TDBFOPEN = HDBFLAGS.HDBFOPEN,                   /* whether opened */
  TDBFFATAL = HDBFLAGS.HDBFFATAL                  /* whetehr with fatal error */
};

enum TDBOPT : ubyte                                    /* enumeration for tuning options */
{
...
enum TDBOMODE : int                                    /* enumeration for open modes */
{
...
enum TDBITYPE                            /* enumeration for index types */
{
  TDBITLEXICAL,                          /* decimal string */
...
struct TDBCOND                          /* type of structure for a condition */
{
  char *name;                            /* column name */
...
struct TDBQRY                          /* type of structure for a query */
{
...
enum TDBQCS                                    /* enumeration for query conditions */
{
...
enum TDBQO                                    /* enumeration for order types */
{
...
enum TDBS                                    /* enumeration for set operation types */
{
  TDBMSUNION,                            /* union */
...
enum TDBQP                                    /* enumeration for post treatments */
{
...
/* Get the last happened error code of a table database object.
   ...
   miscellaneous error. */
TCERRCODE tctdbecode ( TCTDB *tdb );
...
bool tctdbtune ( TCTDB *tdb, long bnum, byte apow, byte fpow, ubyte opts );
...
bool tctdbsetcache ( TCTDB *tdb, int rcnum, int lcnum, int ncnum );
...
bool tctdbsetxmsiz ( TCTDB *tdb, long xmsiz );
...
bool tctdbsetdfunit ( TCTDB *tdb, int dfunit );
...
/* Optimize the file of a table database object.
   ...
   successive updating. */
bool tctdboptimize ( TCTDB *tdb, long bnum, byte apow, byte fpow, ubyte opts );
...
bool tctdbsetindex ( TCTDB *tdb, char *name, TDBITYPE type );


/* Generate a unique ID number of a table database object.
   `tdb' specifies the table database object connected as a writer.
   The return value is the new unique ID number or -1 on failure. */
long tctdbgenuid ( TCTDB *tdb );
...
void tctdbqryaddcond ( TDBQRY *qry, char *name, TDBQCS op, char *expr );
...
void tctdbqrysetorder ( TDBQRY *qry, char *name, TDBQO type );
...
TCLIST *tctdbmetasearch ( TDBQRY **qrys, int num, TDBS type );
...
long tctdbuidseed ( TCTDB *tdb );


/* Set the seed of unique ID unumbers of a table database object.
   `tdb' specifies the table database object connected as a writer.
   If successful, the return value is true, else, it is false. */
bool tctdbsetuidseed ( TCTDB *tdb, long seed );
...
bool tctdbsetinvcache ( TCTDB *tdb, long iccmax, double iccsync );
...
/* Perform dynamic defragmentation of a table database object.
   ...
   If successful, the return value is true, else, it is false. */
bool tctdbdefrag ( TCTDB *tdb, long step );
...
bool tctdbputproc ( TCTDB *tdb, void *pkbuf, int pksiz, void *cbuf, int csiz,
...
/* Generate keyword-in-context strings from a query object.
   ...
   Because the object of the return value is created with the function `tclistnew', it should
   be deleted with the function `tclistdel' when it is no longer in use. */
TCLIST *tctdbqrykwic ( TDBQRY *qry, TCMAP *cols, char *name, int width, TCKWGEN opts );
