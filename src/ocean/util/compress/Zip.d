...
struct LocalFileHeader
{
    ...
    void fill(InputStream src)
    {
        ...
        readExact(src, tmp);
        auto mfile_name = cp437_to_utf8(tmp);
        file_name = assumeUnique(mfile_name);
...
struct FileHeader
{
    ...
    long map(void[] src)
    {
        ...
        cstring function(Const!(ubyte)[]) conv_fn;
        if( usingUtf8 )
            conv_fn = &cp437_to_utf8;
        else
            conv_fn = &utf8_to_utf8;

        auto mfile_name = conv_fn(
        ...
        auto mfile_comment = conv_fn(
                cast(ubyte[]) src[0..data.file_comment_length]);
        file_comment = assumeUnique(mfile_comment);
...
struct EndOfCDRecord
{
    ...
    void fill(void[] src)
    {
        ...
        file_comment = cast(istring) src[0..data.file_comment_length].dup;
    }
}
...
{
    void finish();
    void putFile(ZipEntryInfo info, istring path);
    void putFile(ZipEntryInfo info, istring path);
...
private:
            ...
            istring file_name)
    {
        ...
        header.file_name = assumeUnique(f_name);
...
private:
    ...
    ZipEntry reset(FileHeader header, open_dg_t open_dg)
    {
        ...
        with( info )
        {
            auto mname = Path.standard(header.file_name.dup);
            name = assumeUnique(mname);
...
{
    this(istring msg) { super(msg); }

private:
    alias typeof(this) thisT;
    static void opCall(istring msg) { throw new ZipException(msg); }
    ...
    static void badsig(istring type)
    {
...
private:
    static void opCall(istring name) { throw new ZipChecksumException(name); }
}
...
private:
    ...
    static void opCall(istring msg)
    {
    ...
    {
        throw new thisT(idup("zip format version "
...
void createArchive(istring archive, Method method, istring[] files...)
{
...
void extractArchive(istring archive, istring dest)
{
    ...
    foreach( entry ; zr )
    {
        ...
        auto path_mut = Path.normalize(path);

        // Create the parent directory if necessary.
        auto parent = Path.parse(path_mut).parent;
        if( !Path.exists(parent) )
        {
            Path.createPath(parent);
        }

        path = Path.native(path_mut);
...
cstring cp437_to_utf8(Const!(ubyte)[] s)
{
    foreach( i,c ; s )
    {
        if( (1 <= c && c <= 31) || c >= 127 )
        {
            ...
            foreach( d ; s[i..$] )
            {
                ...
                {
                    istring repl = cp437_to_utf8_map_low[d];
                ...
                {
                    istring repl = cp437_to_utf8_map_high[d-128];
                    r[k..k+repl.length] = repl[];
                    k += repl.length;
                }
                else
...
private char[dchar] _utf8_to_cp437_map;

char[dchar] utf8_to_cp437_map()
{
    return _utf8_to_cp437_map;
}
    ...
    foreach( i,dchar c ; s )
    {
        if( !((32 <= c && c <= 126) || c == 0) )
        {
            ...
            foreach( dchar d ; s[i..$] )
            {
                ...
                {
                    throw new Exception(idup("cannot encode character \""
 ...
 */
cstring utf8_to_utf8(Const!(ubyte)[] s) { return cast(cstring) s; }
Const!(ubyte)[] utf8_to_utf8(cstring s) { return cast(Const!(ubyte)[]) s; }
