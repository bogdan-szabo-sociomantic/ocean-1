...
class ExpiredCacheReloader ( S )
{
    ...
    ***************************************************************************/
    ...
    public static class Cache : ExpiringLRUCache!(CacheValue)
    {
        /***********************************************************************
    ...
    ***************************************************************************/
    ...
    protected Contiguous!(S) load ( hash_t key )
    {
        ...
        auto value_or_null = this.cache_.getAndRefresh(key);
        if (value_or_null !is null)
        {
            auto void_arr = (cast(void*)value_or_null)[0 .. (*value_or_null).sizeof];
            cached_value = CacheValue(void_arr);
        ...
        this.getData(
            ...
            (Contiguous!(S) data)
            {
                ...
                auto value_ptr = this.cache_.getRefreshOrCreate(key);
                auto void_arr = (cast(void*)value_ptr)[0 .. (*value_ptr).sizeof];
                cached_value = CacheValue(void_arr);
...
{
    struct Dummy {}
    ExpiredCacheReloader!(Dummy) loader;
}
