...
extern (C) alias int function(int, X509_STORE_CTX *ctx) SSLVerifyCallback;
...
class PrivateKey
{
    ...
    *******************************************************************************/
    override equals_t opEquals(Object obj)
    {
        ...
        if (pk !is null)
            return cast(equals_t) EVP_PKEY_cmp_parameters(pk._evpKey, this._evpKey);
        return 0;
    }
...
class Certificate
{
    ...
    *******************************************************************************/
    ...
    body
    {
        ...
        if (name)
        {
            addNameEntry(name, "C".ptr, country);
            addNameEntry(name, "ST".ptr, stateProvince);
            addNameEntry(name, "L".ptr, city);
            addNameEntry(name, "O".ptr, organization);
            if (organizationalUnit !is null)
                addNameEntry(name, "OU".ptr, organizationalUnit);
            if (email) // this might have to go after the CN
                addNameEntry(name, "emailAddress".ptr, email);
            addNameEntry(name, "CN".ptr, cn);
        }
        else
            throwOpenSSLError();
        return this;
    ...
    *******************************************************************************/
    ...
    private void addNameEntry(X509_NAME *name, Const!(char)* type, char[] value)
    {
