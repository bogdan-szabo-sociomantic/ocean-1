...
deprecated class HttpClient
{
        ...
        ***********************************************************************/
        ...
        this (RequestMethod method, cstring url)
        {
        ...
        ***********************************************************************/
        ...
        {
                static istring[] versions = ["HTTP/1.0", "HTTP/1.1"];
        ...
        ***********************************************************************/
        ...
        InputBuffer open (RequestMethod method, Pump pump)
        {
            try {
                ...
                paramsOut.produce (
                    (Const!(void)[] p) {
                        if (tokens.readable) tokens.write("&");
                ...
                // read response line
                if (! responseLine.parse (line.get.dup))
                ...
                if (doRedirect)
                    switch (responseLine.getStatus)
                           ...
                           case HttpResponseCode.TemporaryRedirect:
                                ...
                                if (host)
                                    address = new InternetAddress (idup(host), uri.getValidPort);
                                else
        ...
        **********************************************************************/
        ...
        private void error (cstring msg)
        {
                close;
                throw new IOException (idup(msg));
        }
}
