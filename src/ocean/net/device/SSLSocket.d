...
+/
    ...
    *******************************************************************************/
    ...
    override size_t write(Const!(void)[] src)
    {
        ...
        auto bytes = BIO_write(sslSocket, src.ptr, cast(int) src.length);
        if (bytes <= 0)
            return Eof;
...
+/
        auto bytes = BIO_read(sslSocket, dst.ptr, cast(int) dst.length);
        if (bytes <= 0)
            return Eof;
        return cast(size_t) bytes;
    }
...
class SSLServerSocket : ServerSocket
{
    ...
    *******************************************************************************/
    ...
    SSLSocket acceptSSL (SSLSocket recipient = null)
    {
