...
int CACHELIMIT;   // Half the size of the data cache.
int FASTDIVLIMIT; // crossover to recursive division
...
public:
    ...
    void opAssign(ulong u) {
        if (u == 0) data = ZERO;
            ...
            if (uhi==0) {
              auto raw = new BigDigit[1];
              raw[0] = ulo;
              data = raw;
            } else {
              auto raw = new BigDigit[2];
              raw[0] = ulo;
              raw[1] = uhi;
              data = raw;
            }
...
int opCmp(BigUint y)
{
    ...
    auto k = highestDifferentDigit(data, y.data);
...
char [] toDecimalString(int frontExtraBytes)
{
    ...
    auto sofar = biguintToDecimal(buff, data.dup);
...
char [] toHexString(int frontExtraBytes, char separator = 0, int minPadding=0, char padChar = '0')
{
    ...
    if (lenBytes > minPadding) {
        // Strip leading zeros.
        auto maxStrip = lenBytes - minPadding;
...
bool fromDecimalString(cstring s)
{
    //Strip leading zeros
    int firstNonZero = 0;
    while ((firstNonZero < s.length - 1) &&
...
static BigUint addOrSubInt(BigUint x, ulong y, bool wantSub, bool *sign) {
    ...
    if (wantSub) { // perform a subtraction
        if (x.data.length > 2) {
            ...
            auto raw = new BigDigit[ d > uint.max ? 2: 1];
            raw[0] = cast(uint)(d & 0xFFFF_FFFF);
            if (d > uint.max) raw[1] = cast(uint)(d>>32);
            r.data = raw;
        }
    } else {
...
static BigUint pow(BigUint x, ulong y)
{
    ...
    auto firstnonzero = firstNonZeroDigit(x.data);
...
BigDigit [] sub(Const!(BigDigit)[] x, Const!(BigDigit)[] y, bool *negative)
{
    if (x.length == y.length) {
        ...
        auto last = highestDifferentDigit(x, y);
...
// return a + b
BigDigit [] add(Const!(BigDigit)[] a, Const!(BigDigit)[] b) {
    Const!(BigDigit)[] x, y;
...
{
    auto hi = cast(uint)(y >>> 32);
    auto lo = cast(uint)(y& 0xFFFF_FFFF);
    auto len = x.length;
...
void mulInternal(BigDigit[] result, Const!(BigDigit)[] x, Const!(BigDigit)[] y)
{
    ...
    if (2*y.length*y.length <= x.length*x.length) {
        ...
        if (extra * extra * 2 < y.length*y.length) {
            ...
            chunksize += cast(size_t) (extra / cast(double)numchunks);
        ...
        BigDigit [] scratchbuff =
            new BigDigit[karatsubaRequiredBuffSize(cast(uint) maxchunk) + y.length];
        ...
        BigDigit [] scratchbuff = new BigDigit[karatsubaRequiredBuffSize(cast(uint) x.length)];
 ...
 *   also be zero.
 */
void squareInternal(BigDigit[] result, Const!(BigDigit)[] x)
{
  ...
  BigDigit [] scratchbuff = new BigDigit[karatsubaRequiredBuffSize(cast(int) x.length)];
...
mstring biguintToHex(mstring buff, Const!(BigDigit)[] data, char separator=0)
{
    int x=0;
 ...
 */
long biguintToDecimal(char [] buff, BigDigit [] data){
    long sofar = buff.length;
 ...
 */
int biguintFromDecimal(BigDigit [] data, cstring s) {
    ...
    Const!(BigDigit)[] y, BigDigit[] scratchbuff)
{
    ...
    auto y0 = y[0 .. half];
    auto y1 = y[half .. $];
    auto mid = scratchbuff[0 .. half*2];
    auto newscratchbuff = scratchbuff[half*2 .. $];
    auto resultLow = result[0 .. 2*half];
    auto resultHigh = result[2*half .. $];
     // initially use result to store temporaries
    auto xdiff= result[0 .. half];
    auto ydiff = result[half .. half*2];
...
void squareKaratsuba(BigDigit [] result, Const!(BigDigit)[] x, BigDigit [] scratchbuff)
{
...
ptrdiff_t firstNonZeroDigit(Const!(BigDigit)[] x)
{
    ptrdiff_t k = 0;
    while (x[k]==0) {
        ++k;
...
body {
    ...
    uint k = cast(uint) (quotient.length >> 1);
    uint h = cast(uint) (k + v.length);
