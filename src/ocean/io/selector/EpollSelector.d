...
version (linux)
{
    ...
    public class EpollSelector: AbstractSelector
    {
         ...
         *
         * Throws:
         * SelectorException if there are not enough resources to add the
         * conduit to the selector (see AbstractSelector.checkErrno).
         *
         * Examples:
         * ---
        ...
        body
        {
            ...
            if (key !is null)
            {
                if ( !registerModify(conduit, events, attachment) )
                {
                    if ( !registerAdd(conduit, events, attachment) )
                    {
                        checkErrno(__FILE__, __LINE__);
                    }
                }
        ...
        /**
         * Adds a new conduit to the selector.
         *
         * Params:
         * conduit      = Conduit that will be associated to the selector;
         *                must be a valid conduit (i.e. not null and open).
         ...
         */
        private bool registerAdd(ISelectable conduit, Event events, Object attachment = null)
        in
        {
            assert(conduit !is null && conduit.fileHandle() >= 0);
            assert((conduit.fileHandle() in _keys) is null);
        }
        body
        {
            epoll_event     event;
        ...
        /**
         * Modifies the events & attachment of a conduit which is already
         ...
         * true on success, false on error
         */
        private bool registerModify(ISelectable conduit, Event events, Object attachment = null)
        in
        {
            assert(conduit !is null && conduit.fileHandle() >= 0);
            assert((conduit.fileHandle() in _keys) !is null);
        ...
        body
        {
            ...
            {
                // failed, depending on which error occurred, remove the file
                // descriptor from the keys array
                int errorCode = errno;
                switch ( errorCode )
                {
                    case EBADF:
