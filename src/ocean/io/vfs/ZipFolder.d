...
class ZipSubFolder : VfsFolder, VfsSync
{
    ...
    ///
    final VfsFile file(istring path)
    in
    {
        assert( valid );
        assert( !Path.parse(path.dup).isAbsolute );
    }
    body
    {
        istring dir, name;
        {
            auto fp = Path.parse(path.dup);
            auto mpath = fp.path();
            auto mfile = fp.file();
            dir = assumeUnique(mpath);
            name = assumeUnique(mfile);
        }
...
class ZipFolder : ZipSubFolder
{
     ...
     */
    this(istring path, bool readonly=false)
    out { assert( valid ); }
    body
    {
        debug( ZipFolder )
...
else
{
        ...
        auto mp = Path.parse (path.dup).path;
        auto p = assumeUnique(mp);
        foreach( file ; this.tree.catalog )
        {
            if( auto zf = cast(ZipFile) file )
                if( zf.entry.file.zipEntry !is null )
                {
                    tempFile = new TempFile(p, TempFile.Permanent);
                    os = tempFile;
                    debug( ZipFolder )
        ...
        // Now, we can create the archive.
        {
            ...
            foreach( file ; this.tree.catalog )
            {
                auto zei = ZipEntryInfo(file.toString[1..$].dup);
     ...
     * flushed to disk.
     */
    final istring path() { return _path; }
    final istring path(istring v) { return _path = v; } /// ditto
...
private:
    ...
    void resetArchive(istring path, bool readonly=false)
    out { assert( valid ); }
    body
    {
        debug( ZipFolder )
...
else
{
    ...
    this(ZipFolder archive, Entry* parent, istring name)
    in
    {
        assert( archive !is null );
        assert( parent );
    ...
    final void reset(ZipFolder archive, Entry* parent, istring name)
    in
    {
        assert( archive !is null );
        assert( parent );
...
bool single_path_part(istring s)
{
    foreach( c ; s )
        if( c == '/' ) return false;
    return true;
...
istring dir_app(istring dir, istring name)
{
    return dir ~ (dir[$-1]!='/' ? "/" : "") ~ name;
}

void headTail(istring path, out istring head, out istring tail)
{
...
class EventSeekInputStream : InputStream //, IConduit.Seek
{
    ...
    override size_t read(void[] dst)
    {
        ...
        if( callbacks.read ) callbacks.read(cast(uint) result, dst);
        return result;
    }
...
class EventSeekOutputStream : OutputStream //, IConduit.Seek
{
    ...
    override size_t write(Const!(void)[] dst)
    {
        ...
        if( callbacks.write ) callbacks.write(cast(uint) result, dst);
        return result;
    }
