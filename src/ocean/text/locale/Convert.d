...
public cstring formatDateTime (char[] output, Time dateTime, cstring format, IFormatService formatService = null)
{
    return formatDateTime (output, dateTime, format, DateTimeFormat.getInstance(formatService));
}

cstring formatDateTime (char[] output, Time dateTime, cstring format, DateTimeFormat dtf)
{
        /**********************************************************************

        **********************************************************************/

        istring expandKnownFormat(cstring format, ref Time dateTime)
        {
                istring f;
...
version (Full)
{
        ...
        **********************************************************************/
        ...
        cstring formatCustom (ref Result result, Time dateTime, cstring format)
        {

                int parseRepeat(cstring format, int pos, char c)
                {
                        int n = pos + 1;
                ...
                cstring formatDayOfWeek(Calendar.DayOfWeek dayOfWeek, int rpt)
                {
                        if (rpt is 3)
                ...
                cstring formatMonth(int month, int rpt)
                {
                        if (rpt is 3)
                ...
                cstring formatInt (char[] tmp, int v, int minimum)
                {
                ...
                int parseQuote(cstring format, int pos, out char[] result)
                {
                        int start = pos;
...
char[] longToString (char[] buffer, long value, int digits, cstring negativeSign)
{
        if (digits < 1)
            digits = 1;
...
char parseFormatSpecifier (cstring format, out int length)
{
        int     i = -1;
        char    specifier;
...
cstring formatInteger (char[] output, long value, cstring format, NumberFormat nf)
{
        int     length;
...
cstring formatDouble (char[] output, double value, cstring format, NumberFormat nf)
{
        int length;
        int precision = 6;
        Result result = Result (output);
...
{
        cstring format = number.sign ? negativeNumberFormats[nf.numberNegativePattern]
...
{
        cstring format = number.sign ? negativeCurrencyFormats[nf.currencyNegativePattern]
...
void formatFixed (ref Number number, ref Result target, int length,
                  int[] groupSizes, cstring decimalSeparator, cstring groupSeparator)
{
        int pos = number.scale;
        auto p = number.digits.ptr;
...
cstring toString (ref Number number, ref Result result, char format, int length, NumberFormat nf)
{
        switch (format)
               {
               case 'c':
...
private struct Number
{
        ...
        **********************************************************************/
        ...
        private static Number opCall (long value)
        {
                ...
                size_t end = -(n - buffer.length);
                number.scale = cast(int) end;
        ...
        **********************************************************************/
        ...
        private char[] toStringFormat (ref Result result, cstring format, NumberFormat nf)
        {
                bool hasGroups;
                int groupCount;
                ...
                auto groupSeparator = nf.numberGroupSeparator;
                auto decimalSeparator = nf.numberDecimalSeparator;
