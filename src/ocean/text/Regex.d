/*******************************************************************************
    ...
    Copyright:
        ...
        Small block copyright (c) 2003-2004 by Digital Mars, www.digitalmars.com
...
//            assert(p.l_<=p.r_, Int.toString(i)~": "~Int.toString(p.l_)~" > "~Int.toString(p.r_));
    ...
    mixin (genOpCmp("
    {
        if ( parts.length < rhs.parts.length )
            return -1;
        if ( parts.length > rhs.parts.length )
            return 1;
        foreach ( i, p; rhs.parts )
        {
...
private:
    ...
    static istring[] operator_names = ["EOS", "concat", "|", "(", ")", "?", "*", "+", "??", "*?", "+?", "??x", "*?x", "+?x", "(?", "{x,y}", "{x,y}?"];

    /// Actions for to-postfix transformation
    enum Act {
        pua, poc, poa, pca, don, err
    }
    static istring[] action_names = ["push+advance", "pop+copy", "pop+advance", "pop+copy+advance", "done", "error"];
    ...
    state_t addState()
    {
        ...
        s.index = cast(uint) states.length;
        states ~= s;
        return s;
    }
    ...
    state_t clone(state_t s)
    {
        ...
        s2.index = cast(uint) states.length;
        states ~= s2;
        return s2;
    }
    ...
    frag_t constructChars(Const!(cc_t) charclass, predicate_t.Type type)
    {
    ...
    **********************************************************************************************/
    struct Command
    {
        ...
        ******************************************************************************************/
        mixin(genOpCmp("
        {
            if ( src == CURRENT_POSITION_REGISTER && rhs.src != CURRENT_POSITION_REGISTER )
                return 1;
            if ( src != CURRENT_POSITION_REGISTER && rhs.src == CURRENT_POSITION_REGISTER )
                return -1;
            if ( dst < rhs.dst )
        ...
        version (D_Version2)
        {
            // D2: using it as AA key requires more restrictive opEquals in D2
            //     also toHash becomes mandatory
            mixin("
            equals_t opEquals(ref const Command cmd) const
            {
...
class RegExpT(char_t)
{
    ...
    **********************************************************************************************/
    ...
    /**********************************************************************************************
        ...
        Example:
            ...
            ---
    **********************************************************************************************/
    static RegExpT!(char_t) opCall(Const!(char_t)[] pattern, char_t[] attributes = null)
    {
    ...
    **********************************************************************************************/
    ...
    {
        int result;
        auto _this = this; // to avoid passing `this` class references by ref
        while ( !result && test() )
            result = dg(_this);
        return result;
    }
    ...
    **********************************************************************************************/
    Const!(char_t)[] match(uint index)
    {
        if ( index > tdfa_.num_tags )
            return null;
        auto start   = last_start_+registers_[index*2],
    ...
    **********************************************************************************************/
    ...
    /**********************************************************************************************
        ...
        Example:
            ...
            ---
    **********************************************************************************************/
    Const!(char_t)[][] split(char_t[] input)
    {
        auto res = new Const!(char_t)[][PREALLOC];
        uint index;
    ...
    **********************************************************************************************/
    ...
    istring compileToD(cstring func_name = "match", bool lexer=false)
    {
        istring code;
        istring str_type;
        static if ( is(char_t == char) )
...
private:
    ...
    istring compileCommand(tdfa_t.Command cmd, Const!(char_t)[] indent)
    {
        istring  code,
                 dst;
        code ~= Format.convert("\n{}r{} = ", indent, cmd.dst);
