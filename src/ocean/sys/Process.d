...
class Process
{
     ...
     * execution.
     */
    deprecated public void execute(cstring arg1, cstring[] args ...)
    in
    {
        assert(!_running);
    }
     ...
     * instead.
     */
    deprecated public void execute(cstring command, istring[istring] env)
    in
    {
        assert(!_running);
        assert(command.length > 0);
     ...
     * ---
     */
    deprecated public void execute(cstring[] args, istring[istring] env)
    in
    {
        assert(!_running);
        assert(args.length > 0);
